...
class App(ctk.CTk, SetWinLocationMixin):
    ...
    def button_connect(self):
        if self.condition.get() == 'Connect':
            ...
            self.port = comport.ComPortProcessor(self.combo_vars[1][1].get(), self.combo_vars[2][1].get(),
    ...
    def update_textbox(self):
        while not self.port.queue.empty():
            self.all_ans.insert(0.0, ''.join(ch for ch in self.port.queue.get(timeout=1)) + '\n')
...
class SetFil(ctk.CTkToplevel, SetWinLocationMixin):
    ...
    def send(self):
        if self.port:
            self.port.write([int(self.avrg.get()), int(float(self.coef.get())*1000)])
        self.destroy()
